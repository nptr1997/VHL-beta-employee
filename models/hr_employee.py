...
class HrEmployeePrivate(models.Model):
    ...
    # Các trường liên quan tới tài khoản người dùng và tài khoản nhân viên
    ...
    #Thông tin cá nhân-----Bổ sung thêm trường
    children_info = fields.Char(string="Children", groups="hr.group_hr_user", tracking=True)
    children_birth = fields.Char(string="Children birthdate", groups="hr.group_hr_user", tracking=True)
    citizen_iden = fields.Char(string="Citizen identification number", groups="hr.group_hr_user", tracking=True)
    date_of_citizen = fields.Date(string="Date of issue", groups="hr.group_hr_user", tracking=True)
    place_of_citizen = fields.Char(string="Place of issue", groups="hr.group_hr_user", tracking=True)
    permanent_address = fields.Char(string="Permanent address", groups="hr.group_hr_user", tracking=True)
    temporary_address = fields.Char(string="Temporary address", groups="hr.group_hr_user", tracking=True)
    relation_phone = fields.Char(string="Relative phone", groups="hr.group_hr_user", tracking=True)
    #-----Hết
    #Thông tin làm việc-----Bổ sung thêm trường
    employee_code = fields.Char(string="Employee code", groups="hr.group_hr_user", tracking=True)
    employee_position = fields.Selection([
        ...
        ('staff', "Staff"),
        ('manager', "Manager")
    ], "Position", groups="hr.group_hr_user", tracking=True)
    employee_group = fields.Char(string="Workgroup", groups="hr.group_hr_user", tracking=True)
    employee_company_phone = fields.Char(string="Company phone number", groups="hr.group_hr_user", tracking=True)
    employee_onboard_date = fields.Date(string="Onboard date", groups="hr.group_hr_user", tracking=True)
    employee_offboard_date = fields.Date(string="Offboard date", groups="hr.group_hr_user", tracking=True)
    employee_leave = fields.Boolean(string="Leaving", groups="hr.group_hr_user", tracking=True)
    employ_detail_job = fields.Selection([
        ('office', "Office"),
        ('milk_run', "Milkrun"),
        ('accounting', "Accounting"),
        ('ga_hr', "GA&HR"),
        ('sp_loading', "SP-Loading"),
        ('driver', "Driver"),
        ('sp', "SP"),
        ('sp_packing', "SP Packing"),
        ('sp_pa', "SP PA"),
        ('cbu', "CBU"),
        ('clean_box', "Clean box"),
        ('pdi_hai_phong', "PDI Hai phong"),
        ('pdi_vinh_phuc', "SPDI Vinh Phuc"),
        ('tire_assy', "Tire Assy"),
        ('ms', "MS"),
        ('sales', "Sales"),
        ('ikd', "IKD")
    ], "Detail job", groups="hr.group_hr_user", tracking=True)
    employee_work_place = fields.Selection([
        ('VHL_VP_Office', "VHL Vĩnh Phúc - Office"),
        ('VHL_HNA_Katolec', "VHL Hà Nam - Katolec"),
        ('VHL_HNA_Factory', "VHL Hà Nam - Factory"),
        ('VHL_HN', "VHL Hà Nội"),
        ('VHL_VP_Sp', "VHL Vĩnh Phúc - SP")
    ], "Work branch location", groups="hr.group_hr_user", tracking=True)
    employee_bank_account = fields.Char(string="Bank account number", groups="hr.group_hr_user", tracking=True)
    employee_bank = fields.Char(string="Bank", groups="hr.group_hr_user", tracking=True)
    employee_tax_code = fields.Char(string="Tax code", groups="hr.group_hr_user", tracking=True)
    employee_social_insurance = fields.Char(string="Social insurance number", groups="hr.group_hr_user", tracking=True)
    work_contract_id = fields.One2many("hr.work.contract", "employ_id", string="Work Contract")
    # -----Hết
    #Địa chỉ email thông tin làm việc
    ...
    #Tình trạng hôn nhân thông tin cá nhân
    ...
    #Trình độ học vấn thông tin làm việc
